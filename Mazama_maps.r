...
#   bg          background color for oceans
#
...
country_map <- function(countries=c('US'),col=c('firebrick2'),
                        view='auto',resolution='lo',
                        axes=FALSE, bg='white') {

  # These colors should match those in Import_Export_plot.r
  color_export_map = '#40CC90'  
  color_import_map = '#EE5555'
  color_missing_map = 'gray90'
  color_net_0_map = '#9999A9'
...
#South and Central America, Africa, Middle East, Non-OECD Asia and Non-OECD Europe. 
  ...
  # The sp package works with geodata like the following world outlines described here:
  #   http://markmail.org/thread/jhmvmuvslzxemgyl
  require(sp)
  if (resolution == 'lo') {
    load('/var/www/mazamascience.com/html/OilExport/simpleMap.RData')
  } else {
    # default to lo resolution
    load('/var/www/mazamascience.com/html/OilExport/simpleMap.RData')
  }

  # Plot the basemap
  plot(world_countries,axes=FALSE,xlim=xlim,ylim=ylim,bg=bg)
  plot(world_countries,add=TRUE,col=c('white'))

  # Plot each individual country int the appropriate color
